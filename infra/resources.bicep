...
@description('Principal ID (object ID) of the user to assign Grafana Admin. Set automatically by the preprovision hook. Leave empty to skip.')
...
// Grafana system identity needs Monitoring Reader to query AppInsights + Log Analytics
resource grafanaMonitoringReader 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  scope: resourceGroup()
  name: guid(resourceGroup().id, grafana.name, '43d0d8ad-25c7-4714-9337-8ba259a9fe05')
  properties: {
...
// Grant the deploying user Grafana Admin so they can log in and build dashboards
resource grafanaAdminRole 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!empty(grafanaAdminPrincipalId)) {
  scope: grafana
  name: guid(grafana.id, grafanaAdminPrincipalId, '22926164-76b3-42b3-bc55-97df8dab3e41')
  properties: {
